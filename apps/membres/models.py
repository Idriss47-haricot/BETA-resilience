...
class Membre(models.Model):
    ...
    def __str__(self):
        return f'{self.prenom} {self.nom}'.strip() or f'Membre #{self.pk}'

    def save(self, *args, **kwargs):
        # Générer un slug garanti unique
        if not self.slug:
            text_source = f'{self.prenom} {self.nom}'.strip()
            if not text_source and self.user:
                text_source = self.user.username
            if not text_source and self.email:
                text_source = self.email.split('@')[0]

            base_slug = slugify(text_source) if text_source else 'membre'
            slug = base_slug
            
            # Gestion des doublons de slug
            while Membre.objects.filter(slug=slug).exclude(pk=self.pk).exists():
